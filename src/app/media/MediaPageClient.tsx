...
import { galleryImages as fallbackGalleryImages, aicInfo } from "@/data/content";
import {
  Camera,
  Play,
  X,
...
const fallbackPodcasts: PodcastData[] = [
  {
    id: "1",
...
export default function MediaPageClient({
  ...
  const galleryImages = cmsGalleryImages?.length ? cmsGalleryImages : fallbackGalleryImages;
  const videos = cmsVideos?.length ? cmsVideos : fallbackVideos;
  const podcasts = cmsPodcasts?.length ? cmsPodcasts : fallbackPodcasts;
  const latestNews = cmsNews?.length ? cmsNews : fallbackNews;
