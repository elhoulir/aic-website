...
function DonateContent({ causes, frequencies, amounts, bankDetails, impactStats }: DonatePageClientProps) {
  const searchParams = useSearchParams();
  const cancelled = searchParams.get("cancelled");

  const [selectedCause, setSelectedCause] = useState(causes[0]?.id || "general");
  const [selectedFrequency, setSelectedFrequency] = useState(frequencies[0]?.value || "once");
  const [selectedAmount, setSelectedAmount] = useState<number | null>(amounts[2] || 100);
  ...
  const handleDonate = async () => {
    ...
    try {
      const causeTitle = causes.find(c => c.id === selectedCause)?.title || "General Fund";
  ...
  const bankDetailsDisplay = bankDetails ? {
    ...
    details: `BSB: ${bankDetails.bsb || "000-000"} | Account: ${bankDetails.accountNumber || "12345678"}`,
  } : {
    title: "Bank Transfer",
    description: "Make a direct bank transfer to our account.",
              ...
              <div className="bg-white rounded-3xl shadow-xl p-8 md:p-10">
                ...
                <div className="mb-12" id="recurring">
                  ...
                  <p className="text-sm text-gray-500 mt-3 flex items-center gap-2">
                    ...
                    {frequencyDescriptions[selectedFrequency] || "Selected frequency"}
                  </p>
                </div>
...
export default function DonatePageClient(props: DonatePageClientProps) {
  return (
    <Suspense fallback={
