...
function getDisplayDate(event: EventCardProps["event"]): string {
  if (event.recurring) {
...
function EventCard({ event, viewMode, index }: EventCardProps) {
  ...
  if (viewMode === "list") {
    // List view with slide-in details on hover
    return (
      <motion.div
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        {/* Date badge that appears on hover */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-7 gap-2 text-center">
              ...
              {Array.from({ length: 35 }, (_, i) => {
                ...
                // Check for dated events on this day
                const hasEvent = upcomingEvents.some((event) => {
                  if (event.recurring) return false; // Skip recurring events for calendar dots
                ...
                // Check for recurring events on this day of week
                const dayOfWeek = new Date(currentYear, currentMonth, day).getDay();
                const dayNames = ["Sundays", "Mondays", "Tuesdays", "Wednesdays", "Thursdays", "Fridays", "Saturdays"];
                const hasRecurringEvent = isValidDay && upcomingEvents.some((event) => {
